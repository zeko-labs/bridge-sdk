import {
	fetchActionsQuery,
	fetchCommitAsePastSlotQuery,
	fetchInnerActionsQueryIndexer,
	fetchInnerWitnessesFromAuxesQuery,
	fetchLiveActionsQuery,
	fetchOuterActionsQueryIndexer,
	fetchOuterWitnessesFromAuxesQuery,
	networkStateQuery
} from "@zeko-labs/graphql"
import { fetchAccount, Field, Mina, Poseidon, PublicKey, UInt32, type NetworkId } from "o1js"
import * as v from "valibot"
import { Actions, empty, fromList } from "./actions"
import type { GqlClient } from "./graphql"
import type {
	Account,
	Action,
	BridgeVersion,
	Deposit,
	InnerAction,
	InnerWitness,
	OuterAction,
	OuterCommit,
	OuterWitness,
	Withdrawal
} from "./types"

const describeGraphqlError = (error: unknown): string | null => {
	if (error == null) return null
	if (error instanceof Error && error.message) return error.message
	return String(error)
}

const describeValidationIssues = (issues: ReadonlyArray<{ message: string }>): string =>
	issues.map((issue) => issue.message).join("; ")

const createInvalidPayloadError = ({
	label,
	error,
	issues
}: {
	label: string
	error: unknown
	issues: ReadonlyArray<{ message: string }>
}): Error => {
	const details = [describeGraphqlError(error), describeValidationIssues(issues)].filter(
		(detail): detail is string => detail != null && detail.length > 0
	)

	return new Error(details.length > 0 ? `${label}: ${details.join("; ")}` : label)
}

const parsePayload = <TSchema extends v.BaseSchema<unknown, unknown, v.BaseIssue<unknown>>>({
	schema,
	input,
	error,
	label
}: {
	schema: TSchema
	input: unknown
	error: unknown
	label: string
}): v.InferOutput<TSchema> => {
	const result = v.safeParse(schema, input)
	if (!result.success) {
		throw createInvalidPayloadError({
			label,
			error,
			issues: result.issues
		})
	}

	return result.output
}

const indexedActionEntries = {
	afterState: v.string(),
	beforeState: v.nullable(v.string()),
	index: v.number(),
	blockHeight: v.number(),
	timestamp: v.string(),
	txnHash: v.string(),
	fields: v.array(v.string())
}

const InnerActionSchema = v.object({
	...indexedActionEntries,
	aux: v.string()
})

const innerActionFromSchema = (x: v.InferOutput<typeof InnerActionSchema>) => ({
	beforeActionState: Field.from(x.beforeState ?? empty().hash),
	afterActionState: Field.from(x.afterState),
	timestamp: x.timestamp,
	hash: x.txnHash,
	index: x.index,
	action: x.fields.map(Field.from),
	aux: Field.from(x.aux)
})

const BaseOuterActionSchema = v.object({
	...indexedActionEntries,
	slotRangeLower: v.string(),
	slotRangeUpper: v.string(),
	kind: v.string()
})

const baseActionFromSchema = (x: v.InferOutput<typeof BaseOuterActionSchema>) => ({
	beforeActionState: Field.from(x.beforeState ?? empty().hash),
	afterActionState: Field.from(x.afterState),
	timestamp: x.timestamp,
	hash: x.txnHash,
	index: x.index,
	action: x.fields.map(Field.from),
	slotRange: {
		lower: UInt32.from(x.slotRangeLower),
		upper: UInt32.from(x.slotRangeUpper)
	}
})

const CommitSchema = v.object({
	...BaseOuterActionSchema.entries,
	kind: v.literal("COMMIT"),
	ledger: v.string(),
	innerActionState: v.string(),
	innerActionStateLength: v.number(),
	synchronizedOuterActionState: v.string(),
	synchronizedOuterActionStateLength: v.number()
})

const commitFromSchema = (x: v.InferOutput<typeof CommitSchema>) => ({
	ledger: Field.from(x.ledger),
	innerActionState: Field.from(x.innerActionState),
	innerActionStateLength: UInt32.from(x.innerActionStateLength),
	synchronizedOuterActionState: Field.from(x.synchronizedOuterActionState),
	synchronizedOuterActionStateLength: UInt32.from(x.synchronizedOuterActionStateLength)
})

const WitnessSchema = v.object({
	...BaseOuterActionSchema.entries,
	kind: v.literal("WITNESS"),
	aux: v.string()
})

const witnessFromSchema = (x: v.InferOutput<typeof WitnessSchema>) => ({
	aux: Field.from(x.aux)
})

const OuterActionSchema = v.variant("kind", [CommitSchema, WitnessSchema])
const OuterActionsSchema = v.array(OuterActionSchema)
const InnerActionsSchema = v.array(InnerActionSchema)
const OuterWitnessesSchema = v.array(WitnessSchema)
const CommitAsePastSlotSchema = v.object({
	commit: v.nullable(CommitSchema),
	ase: OuterActionsSchema
})

const outerActionFromSchema = (action: v.InferOutput<typeof OuterActionSchema>): OuterAction => ({
	...baseActionFromSchema(action),
	...(action.kind === "COMMIT"
		? { ...commitFromSchema(action), type: "commit" }
		: { ...witnessFromSchema(action), type: "witness" })
})

export async function fetchOuterActionsFromIndexer(
	client: GqlClient,
	options: { fromState: string; afterState?: string | null | undefined }
): Promise<OuterAction[]> {
	const { data, error } = await client().query(fetchOuterActionsQueryIndexer, {
		beforeState: options.fromState,
		afterState: options.afterState
	})

	const actions = parsePayload({
		schema: OuterActionsSchema,
		input: data?.outerActions,
		error,
		label: "Invalid actions data"
	})

	return actions.map(outerActionFromSchema)
}

export async function fetchInnerActionsFromIndexer(
	client: GqlClient,
	options: { fromState: string; afterState?: string | null | undefined }
): Promise<InnerAction[]> {
	const { data, error } = await client().query(fetchInnerActionsQueryIndexer, {
		beforeState: options.fromState,
		afterState: options.afterState
	})

	const actions = parsePayload({
		schema: InnerActionsSchema,
		input: data?.innerActions,
		error,
		label: "Invalid actions data"
	})

	return actions.map(innerActionFromSchema)
}

export async function fetchOuterWitnessesFromAuxes(
	client: GqlClient,
	auxes: string[]
): Promise<OuterWitness[]> {
	const { data, error } = await client().query(fetchOuterWitnessesFromAuxesQuery, { auxes })

	const witnesses = parsePayload({
		schema: OuterWitnessesSchema,
		input: data?.outerWitnessesFromAuxes,
		error,
		label: "Invalid witnesses data"
	})

	return witnesses.map((witness) => ({
		...baseActionFromSchema(witness),
		...witnessFromSchema(witness),
		type: "witness"
	}))
}

export async function fetchInnerWitnessesFromAuxes(
	client: GqlClient,
	auxes: string[]
): Promise<InnerWitness[]> {
	const { data, error } = await client().query(fetchInnerWitnessesFromAuxesQuery, { auxes })

	const witnesses = parsePayload({
		schema: InnerActionsSchema,
		input: data?.innerWitnessesFromAuxes,
		error,
		label: "Invalid witnesses data"
	})

	return witnesses.map(innerActionFromSchema)
}

export async function fetchCommitAsePastSlot(
	client: GqlClient,
	slot: number
): Promise<{ commit: OuterCommit | null; ase: OuterAction[] }> {
	const { data, error } = await client().query(fetchCommitAsePastSlotQuery, { slot })

	const commitAsePastSlot = parsePayload({
		schema: CommitAsePastSlotSchema,
		input: data?.commitAsePastSlot,
		error,
		label: "Invalid commit ASE past withdrawal delay data"
	})

	return {
		commit:
			commitAsePastSlot.commit === null
				? null
				: {
						...baseActionFromSchema(commitAsePastSlot.commit),
						...commitFromSchema(commitAsePastSlot.commit),
						type: "commit"
					},
		ase: commitAsePastSlot.ase.map(outerActionFromSchema)
	}
}

// Archive queries are most reliable in recent 10k-block windows.
// Avoid a giant from=0 scan when investigating missing actions manually.
// The graphQL schema server will limit the block scan range to a fixed number of blocks.
// https://github.com/o1-labs/Archive-Node-API/blob/2d726955502fbd02c70d388f589284dd838bbb45/src/server/server.ts#L13
const BLOCK_RANGE_SIZE = 10_000

export const BRIDGE_DEPLOY_BLOCK_BY_NETWORK = {
	testnet: 430_000,
	mainnet: 0
} as const satisfies Record<Extract<NetworkId, "testnet" | "mainnet">, number>

export const getBridgeDeployBlock = (network: NetworkId): number =>
	network === "testnet"
		? BRIDGE_DEPLOY_BLOCK_BY_NETWORK.testnet
		: BRIDGE_DEPLOY_BLOCK_BY_NETWORK.mainnet

export async function getNetworkTip(client: GqlClient): Promise<number> {
	const res = await client().query(networkStateQuery, {})
	if (res.error) throw res.error
	const height = res.data?.networkState?.maxBlockHeight?.pendingMaxBlockHeight ?? 0
	return height
}

export async function fetchActions(
	client: GqlClient,
	pk: PublicKey,
	stopAtBlock = 0
): Promise<Action[]> {
	const networkTip = await getNetworkTip(client)
	const fetchActionsRecursive = async (
		toBlock: number
	): Promise<v.InferOutput<typeof ActionOutputsSchema>> => {
		if (toBlock < stopAtBlock) return []

		const { data, error } = await client().query(fetchActionsQuery, {
			pk: pk.toBase58(),
			from: toBlock - BLOCK_RANGE_SIZE,
			to: toBlock
		})
		if (error) throw error
		const result = parseActionOutputs(data?.actions)
		const prevOutput: v.InferOutput<typeof ActionOutputsSchema> = await fetchActionsRecursive(
			toBlock - BLOCK_RANGE_SIZE
		)
		return [...prevOutput, ...result]
	}

	const output = await fetchActionsRecursive(networkTip)

	return mapParsedActions(output)
}

const ActionOutputsSchema = v.array(
	v.object({
		blockInfo: v.object({ timestamp: v.string(), height: v.number() }),
		actionState: v.object({
			actionStateOne: v.string(),
			actionStateTwo: v.string()
		}),
		actionData: v.array(
			v.object({
				data: v.array(v.string()),
				transactionInfo: v.object({ hash: v.string() })
			})
		)
	})
)

const parseActionOutputs = (actions: unknown): v.InferOutput<typeof ActionOutputsSchema> => {
	const dataWithDefault = (Array.isArray(actions) ? actions : []).map((action) => ({
		...action,
		actionData: Array.isArray(action?.actionData)
			? action.actionData.map(
					(actionData: { transactionInfo?: { hash?: string } } & Record<string, unknown>) => ({
						...actionData,
						transactionInfo: actionData?.transactionInfo ?? { status: "status", hash: "hash" }
					})
				)
			: [],
		blockInfo: {
			timestamp: String(action?.blockInfo?.timestamp ?? "0"),
			height: Number(action?.blockInfo?.height ?? 0)
		}
	}))

	const result = v.safeParse(ActionOutputsSchema, dataWithDefault)
	if (!result.success) throw new Error("Invalid actions data")
	return result.output
}

const mapParsedActions = (output: v.InferOutput<typeof ActionOutputsSchema>): Action[] =>
	output.flatMap(
		(
			{ actionState: { actionStateTwo: actionStateBefore }, actionData, blockInfo: { timestamp } },
			index
		) => {
			const actions = actionData.map(({ data, transactionInfo: { hash } }) => ({
				data: data.map(Field.from),
				hash
			}))
			let actionState = Field.from(actionStateBefore)

			const correctActions = []
			for (let i = 0; i < actions.length; i++) {
				const action = actions[i]
				const afterActionState = Actions.pushHash(actionState, fromList([action.data]).hash)
				correctActions.push({
					index: index + i,
					action: action.data,
					beforeActionState: actionState,
					afterActionState,
					timestamp,
					hash: action.hash
				})
				actionState = afterActionState
			}

			return correctActions
		}
	)

export async function fetchRecentActions(
	client: GqlClient,
	pk: PublicKey,
	fromActionState?: string
): Promise<Action[]> {
	const { data, error } = await client().query(fetchLiveActionsQuery, {
		pk: pk.toBase58(),
		fromActionState: fromActionState ?? null
	})
	if (error) throw error
	const output = parseActionOutputs(data?.actions)

	return mapParsedActions(output)
}

export function depositAux(deposit: Deposit): Field {
	return Poseidon.hashWithPrefix("Deposit_params - qFB3jXP*)", [
		Field(0), // Empty children
		...deposit.holderAccountL1.toFields(),
		...deposit.amount.toFields(),
		...deposit.recipient.toFields(),
		...deposit.timeout.toFields()
	])
}

export function withdrawalAux(withdrawal: Withdrawal): Field {
	return Poseidon.hashWithPrefix("Withdrawal_params - qFB3jXP*)", [
		Field(0), // Empty children
		...withdrawal.amount.toFields(),
		...withdrawal.recipient.toFields()
	])
}

export function getNextDepositIndex(helperAccount: Account): UInt32 {
	const nextDepositIndex = helperAccount.zkappState?.at(0)
	return UInt32.from(nextDepositIndex ?? 0)
}

export function getNextWithdrawalIndex(helperAccount: Account): UInt32 {
	const nextWithdrawalIndex = helperAccount.zkappState?.at(1)
	return UInt32.from(nextWithdrawalIndex ?? 0)
}

export function getNextCancelledDepositIndex(helperAccount: Account): UInt32 {
	const nextCancelledDepositIndex = helperAccount.zkappState?.at(0)
	return UInt32.from(nextCancelledDepositIndex ?? 0)
}

export function filterNulls<T>(arr: (T | null)[]): T[] {
	return arr.filter(Boolean).map((x) => {
		if (x === null) throw new Error("Unreachable")
		return x
	})
}

export async function refreshCache(feePayer: Mina.FeePayerSpec, tokenId?: Field) {
	if (feePayer instanceof PublicKey) {
		await fetchAccount({ publicKey: feePayer, tokenId })
	} else if (feePayer) {
		await fetchAccount({
			publicKey: feePayer.sender,
			tokenId
		})
	}
}

export function safeDecrement(a: UInt32): UInt32 {
	return a.greaterThanOrEqual(UInt32.from(1)).toBoolean() ? a.sub(UInt32.from(1)) : a
}

export function checkAccepted(
	outerActions: OuterAction[],
	depositIndex: UInt32,
	depositParams: Deposit
) {
	let isRejected = false
	let isAccepted = false

	for (const action of outerActions) {
		if (isRejected || isAccepted) break

		const slotRange = action.slotRange

		isRejected =
			isRejected || (!isAccepted && slotRange.lower.greaterThan(depositParams.timeout).toBoolean())

		const depositIndexBeforeCommit =
			action.type === "commit"
				? action.synchronizedOuterActionStateLength.greaterThan(depositIndex).toBoolean()
				: false

		isAccepted =
			isAccepted ||
			(!isRejected &&
				slotRange.upper.lessThanOrEqual(depositParams.timeout).toBoolean() &&
				depositIndexBeforeCommit &&
				action.type === "commit")
	}

	return { isRejected, isAccepted }
}

export async function getVkHash(publicKey: PublicKey): Promise<Field> {
	await refreshCache(publicKey)
	const vk = Mina.getAccount(publicKey).zkapp?.verificationKey?.hash
	if (!vk) throw new Error("Verification key not found")
	return Field(vk)
}

export function uint32Max(a: UInt32, b: UInt32): UInt32 {
	return a.greaterThan(b).toBoolean() ? a : b
}

export function bridgeVersionForActionIndex({
	actionIndex,
	v2StartIndex
}: {
	actionIndex: number
	v2StartIndex: UInt32
}): BridgeVersion {
	if (actionIndex < 0) return 2

	return v2StartIndex.greaterThan(UInt32.from(actionIndex)).toBoolean() ? 1 : 2
}
